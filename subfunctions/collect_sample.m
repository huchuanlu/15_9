function [sample label sz img_sear] = collect_sample(img, sz, p,rangetemp)
...
%% 加入梯度特征
[height width depth] = size(sample);

    gimage = sample;

filter=fspecial('gaussian',3,0.5);
gimage = conv2(gimage, filter, 'same'); 

filter = [-1 0 1];       
%horizontal edge
himage = conv2(gimage, filter, 'same');
%vertical edge
vimage = conv2(gimage, filter', 'same');

% fimage = sqrt(himage.^2 + vimage.^2);

%% 2阶梯度特征加入
filter = [-1 2 -1];
%horizontal 2nd derivative
shimage = conv2(gimage, filter, 'same');
%vertical 2nd derivative     
svimage = conv2(gimage, filter', 'same');
fimage = zeros(height, width, 2);
fimage(:,:,1)   = abs(himage);
fimage(:,:,2)   = abs(vimage);
fimage(:,:,3)   = abs(shimage);
fimage(:,:,4)   = abs(svimage);
...
for i=1:size(sample,1)
    temp=sample(i,:);
    sample(i,:)=255*(temp-min(temp))./(max(temp)-min(temp));
end
